 ...
 * @class describe
 */
class MyAuctionsFragment : EasyBaseFragment() {
    ...
    private fun netData() {
        ...
        EasyHttpManager.getInstance().getService(BoxService::class.java).apiNameMyOver(params).enqueue(object : Callback<NamesModel?> {
            override fun onResponse(call: Call<NamesModel?> , response: Response<NamesModel?>) {
                ...
                if (page == 1 && arrayList.size == 0) {
                    tv_hint.text = "Current account is not registered"
