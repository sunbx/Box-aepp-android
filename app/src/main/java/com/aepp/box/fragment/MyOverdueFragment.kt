 ...
 * @class describe
 */
class MyOverdueFragment : EasyBaseFragment() {
    ...
    private fun netData() {
        ...
        EasyHttpManager.getInstance().getService(BoxService::class.java).apiNameMyRegister(params).enqueue(object : Callback<NamesModel?> {
            override fun onResponse(call: Call<NamesModel?> , response: Response<NamesModel?>) {
                ...
                if (page == 1 && arrayList.size == 0) {
                    tv_hint.text = "There is no domain name to expire \n for the current account"
    ...
    internal inner class AucionsAdapter : BaseAdapter() {
        ...
        override fun getView(position: Int , convertView: View? , parent: ViewGroup?): View {
            ...
            holder.tvTime.text = Utils.formatDateTime((names[position].over_height - names[position].current_height) * 3 * 60)
